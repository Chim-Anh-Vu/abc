...
class FieldElement:
    '''
    Xây dựng trường hữu hạn trên vành nguyên tố p (Zp)
    Định nghĩa lại các phép toán trên vành nguyên tố p (Zp)
    a + b := (a+b) % p
    a - b := (a-b) % p
    a*b := a*b % p
    a/b := a * (b^-1) % p (dùng định lý ferma để tìm nghịch đảo của 1 số trên vành nguyên tố p)
    '''
    def __init__(self, num, prime):
        self.num = num
        self.prime = prime
        self.num = (self.num + self.prime) % self.prime
...
class Point:
    '''
    Định nghĩa đường cong ECC (Elliptic curve Cryptography)
    Định nghĩa lại các phép toán trên đường con ECC
    '''
...
class S256Point(Point):
    ...
    def get_data_as_str(self):
        return str(self.x.get_num()) + '|' + str(self.y.get_num()) + '|' \
            + str(self.a.get_num()) + '|' + str(self.b.get_num())
...
class PrivateKey:
    def __init__(self, secret = None):
      self.secret = secret if secret is not None else randint(0, 2**256) # khóa bí mật a
      self.point = self.secret*G #khóa công khai A
...
class PrivateKeyTest(TestCase):
    ...
    def test_wif(self):
        ...
        res = pk.wif(compressed=True, testnet=False)
        self.assertEqual(res, expected)
        self.assertEqual(PrivateKey.parse(res, compressed=True, testnet=False).secret, pk.secret)
